...
def _write_src_skeleton(cfg: ProjectConfig, target: Path) -> None:
    ...
    # Agent installers create their own prompts/ directory — skip the base stub
    if not cfg.is_agent:
...
def _write_readme(cfg: ProjectConfig, target: Path) -> None:
    types_str = " + ".join(cfg.app_types)
    if cfg.is_rest_api:
        run_cmd = "uv run uvicorn main:app --reload --port 3100"
    elif cfg.is_teams:
        run_cmd = "uv run python main.py"
    elif cfg.is_batch:
        run_cmd = f"uv run python -m {cfg.pkg_name}.cli --help"
    else:
        run_cmd = f"uv run python -m {cfg.pkg_name}"

    (target / "README.md").write_text(f"""# {cfg.name}

{types_str} — generated by [new-ai-app](https://github.com/luisalmeidabranco/create-ai-app).
...
- `config/config.yaml` — LLM model and app settings
""")
